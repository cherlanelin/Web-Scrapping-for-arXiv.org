...
### Function for getting the information of the first(oldest) N papers from https://arxiv.org/list/cs/new
```{r}
...
### Function for getting the information of the latest N papers from https://arxiv.org/list/cs/new
#### The latest papers are added at the latter of the pagge
```{r}
...
```{r}
paper.first10 = get.first.N.paper(10)
paper.last10 = get.last.N.paper(10)
paper.all = get.all.paper()
```

### Create meta dataset for airflow, just need to run at the first time
```{r}
# pg <- dbDriver("PostgreSQL")
# con <- dbConnect(pg, host="localhost", dbname = "airflow" ,user="postgres",port=5432, password = "aptx0330")
# dbWriteTable(con,'cs_paper_meta',paper, append = TRUE, row.names=FALSE)
# dbDisconnect(con)  
```
...
```{r}
# Based on customer's selection, you can decide updating N latest/first submission papers, or updating all the papers in the website 
...
main.paper.update(n=300, all = TRUE)
```


### Testing: check imported meta data of the papers
```{r}
pg <- dbDriver("PostgreSQL")

## Rewrite the infomration of your postgreSQL
con <- dbConnect(pg, host="localhost", dbname = "airflow" ,user="postgres",port=5432, password = "aptx0330")
